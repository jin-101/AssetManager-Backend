...
public class SavingsTest {
	...
	void insertSavingsOption() throws IOException {
		...
		for (int i = 0; i < list_cnt; i++) {
			...
			if (savings != null) {
				SavingsOptionDTO savingsOption = SavingsOptionDTO.builder().savings(savings)
						...
						.intrRate(jsonObj.get("intr_rate").toString().equals("null") ? null
								: Double.parseDouble(jsonObj.get("intr_rate").toString()))
						.intrRate2(jsonObj.get("intr_rate2").toString().equals("null") ? null
								: Double.parseDouble(jsonObj.get("intr_rate2").toString()))
						.build();
	...
	void insertSavings() throws IOException {
		...
		for (int i = 0; i < list_cnt; i++) {
			...
			if (finCo == null) {
				finCo = FinancialCompanyDTO.builder().finCoNo((String) jsonObj.get("fin_co_no"))
						.korCoNm((String) jsonObj.get("kor_co_nm")).build();
				finCoRepo.save(finCo);
			}

			SavingsDTO save = SavingsDTO.builder().savingsId((i + 1) + "").finCo(finCo)
					.finPrdtNm(jsonObj.get("fin_prdt_nm").toString()).finPrdtCd(jsonObj.get("fin_prdt_cd").toString())
					.joinWay(jsonObj.get("join_way").toString()).mtrtInt(jsonObj.get("mtrt_int").toString())
					.spclCnd(jsonObj.get("spcl_cnd").toString()).joinDeny(jsonObj.get("join_deny").toString())
					.joinMember(jsonObj.get("join_member").toString()).etcNote(jsonObj.get("etc_note").toString())
					.maxLimit(jsonObj.get("max_limit").toString().equals("null") ? null
							: Integer.parseInt(jsonObj.get("max_limit").toString()))
					.dclsStrtDay(convertToDate(dateFormater((String) jsonObj.get("dcls_strt_day"))))
					.dclsEndDay(jsonObj.get("dcls_end_day").toString().equals("null") ? null
							: convertToDate(dateFormater((String) jsonObj.get("dcls_end_day"))))
					.build();
